			...
			<div class="card">
				...
				<div class="card-body">
					<div class="px-3">
					    ...
					    if(isset($_GET['edit']))
					    {
							if($username != 'admin' && $check_menu_banner['edit'] !=1 ){?>
								<script>
...
if($_FILES["pimg"]["name"] == '')
{
    $con->query("update banner set cid=".$cid.", created_by =".$admin_id." where id=".$_GET['edit']."");
...
else 
{
  ...
  $(document).ready(function() {
    ...
    $con->query("update banner set cid=".$cid.",bimg='".$target_file."', created_by =".$admin_id." where id=".$_GET['edit']."");
  ...
  $(document).ready(function() {
    ...
    setTimeout(function()
	{
		...
		<?php 
					    ...
					    {
							if($username != 'admin' && $check_menu_banner['create'] !=1 ){?>
								<script>
									window.location.href="404.php";
								</script>
							<?php	}
...
$target_file = $target_dir . basename($fname);
$cid = $_POST['scat'];
		$con->query("insert into banner(`bimg`,`cid`,`created_by`)values('".$url."',".$cid.",".$admin_id.")");
		move_uploaded_file($_FILES["pimg"]["tmp_name"], $target_file);
		?>
		 <script type="text/javascript">
  $(document).ready(function() {
