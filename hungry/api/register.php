...
if($data['name'] == '' || $data['email'] == '' || $data['mobile'] == '' || $data['imei']==''  || $data['password'] == '' || $data['ccode'] == '' || $data['area'] == '')
{
    ...
    else
    {
       if($refercode != '')
	   {
			...
			if($c_refer != 0)
			{
				...
				$con->query("insert into user(`name`,`imei`,`email`,`mobile`,`rdate`,`password`,`ccode`,`code`,`refercode`,`wallet`,`area_id`)values('".$name."','".$imei."','".$email."','".$mobile."','".$timestamp."','".$password."','".$ccode."','".$prentcode."','".$refercode."',".$fin.",".$area.")");
		...
		else 
		{
			$timestamp = date("Y-m-d H:i:s");
			$prentcode = generate_random();
			$con->query("insert into user(`name`,`imei`,`email`,`mobile`,`rdate`,`password`,`ccode`,`code`,`area_id`)values('".$name."','".$imei."','".$email."','".$mobile."','".$timestamp."','".$password."','".$ccode."','".$prentcode."',".$area.")");
