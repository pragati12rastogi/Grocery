			...
			<div class="card">
				...
				<div class="card-body">
					<div class="px-3">
					    ...
					    if(isset($_GET['eid']))
					    {
							if($username != 'admin' && $check_menu_home['edit'] !=1 ){?>
								<script>
...
$status = $_POST['status'];
$id = $_GET['eid'];
		$con->query("update home set title='".$title."',cid=".$cid.",sid=".$sid.",status=".$status.",created_by=".$admin_id." where id=".$id."");
  ...
  $(document).ready(function() {
   ...
   setTimeout(function(){ window.location.href="home_section_list.php"; }, 1500);
		...
		<?php 
					    ...
					    {
							if($username != 'admin' && $check_menu_home['create'] !=1 ){?>
								<script>
									window.location.href="404.php";
								</script>
							<?php	}
...
$cid = $_POST['catname'];
$status = $_POST['status'];
		$con->query("insert into home(`title`,`cid`,`sid`,`status`,created_by)values('".$title."',".$cid.",".$sid.",".$status.",".$admin_id.")");
