...
<?php if(isset($_GET['edit'])) {
	if($username != 'admin' && $check_sub_category['edit'] !=1 ){
		?>
		<script>
							...
							<div class="form-body">
								...
								<div class="form-group">
									...
									if(isset($check_category) && ($check_category['view_g'] || $username == 'admin')){
										...
										$sel = $con->query("select * from category where created_by =1");
									}
...
$con->query("update subcategory set name='".$cname."',img='".$url."',cat_id=".$cid.", created_by = ".$admin_id." where id=".$_GET['edit']."");
 
}
else 
{
    $con->query("update subcategory set name='".$cname."',cat_id=".$cid.", created_by = ".$admin_id." where id=".$_GET['edit']."");
}
?>
...
<?php } else {
	if($username != 'admin' && $check_sub_category['create'] !=1 ){?>
		<script>
			window.location.href="404.php";
		</script>
	<?php	}	 ?>
...
<div class="form-group">
        ...
        $url = $uploadPath."thump_".$resizeFileName.".". $fileExt;
$con->query("insert into subcategory(`cat_id`,`name`,`img`,created_by)values(".$cid.",'".$cname."','".$url."',".$admin_id.")");
?>
