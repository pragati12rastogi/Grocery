			...
			<div class="card">
				...
				<div class="card-body">
					<div class="px-3">
					    ...
					    if(isset($_GET['edit']))
					    {
							if($username != 'admin' && $check_menu_country['edit'] !=1 ){?>
								<script>
...
$url = $_POST['ccode'];
    ...
    $con->query("update code set status=".$status.",ccode='".$url."',created_by = ".$admin_id." where id=".$_GET['edit']."");
  ...
  $(document).ready(function() {
    ...
    setTimeout(function()
	{
		...
		<?php 
					    ...
					    {
							if($username != 'admin' && $check_menu_country['create'] !=1 ){?>
								<script>
									window.location.href="404.php";
								</script>
							<?php	}
...
$status = $_POST['status'];
$url = $_POST['ccode'];
		$con->query("insert into code(`ccode`,`status`,`created_by`)values('".$url."',".$status.",".$admin_id.")");
				?>
		 <script type="text/javascript">
  $(document).ready(function() {
    toastr.options.timeOut = 4500; // 1.5s
