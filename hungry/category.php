...
<?php if(isset($_GET['edit'])) {
  if($username != 'admin' && $check_category['edit'] !=1 ){
?>
<script>
...
$con->query("update category set catname='".$cname."',catimg='".$url."',created_by = ".$admin_id." where id=".$_GET['edit']."");
 
}
else 
{
    $con->query("update category set catname='".$cname."',created_by = ".$admin_id." where id=".$_GET['edit']."");
}
?>
...
<?php } else { if($username != 'admin' && $check_category['create'] !=1 ){ ?>
    <script>
      window.location.href="404.php";
    </script>
  <?php	}	?>
<div class="row">
        ...
        $url = $uploadPath."thump_".$resizeFileName.".". $fileExt;
$con->query("insert into category(`catname`,`catimg`,created_by)values('".$cname."','".$url."',".$admin_id.")");
?>
