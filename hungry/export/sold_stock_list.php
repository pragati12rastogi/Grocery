    ...
    include '../include/dbconfig.php';
        ...
        if(isset($_GET['export'])){
            ...
            if($username == 'admin' || $check_menu_stock['view_g'] ==1){
                $history = $con->query("SELECT inventory_stock_history.*,category.catname, subcategory.name as subcat from `inventory_stock_history` left join category on category.id = inventory_stock_history.cat_id left join subcategory on inventory_stock_history.subcat_id = subcategory.id where inventory_stock_history.status='Sold' order by id desc ")->fetch_all(MYSQLI_ASSOC);

            }else{
                $history = $con->query("SELECT inventory_stock_history.*,category.catname, subcategory.name as subcat from `inventory_stock_history` left join category on category.id = inventory_stock_history.cat_id left join subcategory on inventory_stock_history.subcat_id = subcategory.id where inventory_stock_history.status='Sold' and inventory_stock_history.created_by =".$admin_id." order by id desc ")->fetch_all(MYSQLI_ASSOC);
            }
            ...
            $file_name = 'sold_stocklist_'.strtotime('now').'.csv';
