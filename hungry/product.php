...
if(isset($_GET['edit']))
{
	if($username != 'admin' && $check_menu_prod['edit'] !=1 ){?>
		<script>
						...
						<form class="form" method="post" enctype="multipart/form-data">
							<div class="form-body">
								...
								<div class="form-group">
									...
									<select id="vname" name="pname"  class="form-control select2" required>
										<option value="" selected="" disabled="">Select Product</option>
												
									<?php 
									if((isset($check_menu_stock) && ($check_menu_stock['view_g']) || $username == 'admin')){
										$pr_name = $con->query("select * from inventory_stock ");
									}else if(isset($check_menu_stock) && $check_menu_stock['view_o']){
										$pr_name = $con->query("select * from inventory_stock where created_by =".$admin_id);
									}else{
										$pr_name = $con->query("select * from inventory_stock where created_by =1");
									}
									while($h = mysqli_fetch_assoc($pr_name))
									{
										$selected = '';
										if($selk['pname'] == $h['product_name']){
											$selected ="selected";
										}
									?>
										<option value="<?php echo $h['product_name'];?>" <?php echo $selected; ?> ><?php echo $h['product_name'];?></option>
									<?php } ?>
									</select>
								</div>
                               ...
                               <div class="form-group">
								...
								<div class="form-group">
											...
											<select id="cat_change" name="catname" class="form-control">
												...
												<?php 
												if(isset($check_category) && ($check_category['view_g'] || $username == 'admin')){
													$sk = $con->query("select * from category where id !=".$selk['cid']."");
												}else if(isset($check_category) && $check_category['view_o']){
													$sk = $con->query("select * from category where created_by =".$admin_id." and id !=".$selk['cid']);
												}else{
													$sk = $con->query("select * from category where created_by =1 and id !=".$selk['cid']."");
												}
...
{
	if($username != 'admin' && $check_menu_prod['create'] !=1 ){?>
		<script>
			window.location.href="404.php";
		</script>
	<?php	}
						...
						<form class="form" action="#" method="post" enctype="multipart/form-data">
							<div class="form-body">
								...
								<div class="form-group">
									...
									<?php 
									if(isset($check_menu_stock) && ($check_menu_stock['view_g'] || $username == 'admin')){
										$pr_name = $con->query("select * from inventory_stock where prod_quantity > 0");
									}else{
										$pr_name = $con->query("select * from inventory_stock where prod_quantity > 0 and created_by =".$admin_id);
									}
									...
									?>
										<option value="<?php echo $h['product_name'];?>"><?php echo $h['product_name'];?></option>
									<?php } ?>
									</select>
								</div>
								...
								<div class="form-group">
											...
											<select id="cat_change" name="catname" class="form-control" required>
												...
												<?php 
												if(isset($check_category) && ($check_category['view_g'] || $username == 'admin')){
													$sk = $con->query("select * from category");
												}else if(isset($check_category) && $check_category['view_o']){
													$sk = $con->query("select * from category where created_by =".$admin_id);
												}else{
													$sk = $con->query("select * from category where created_by =1");
												}
												while($h = mysqli_fetch_assoc($sk))
												{
												?>
...
else 
{
	...
	$arr[] = "product/".$file_name;
		...
		$con->query("insert into product(`pname`,`pimg`,`prel`,`sname`,`cid`,`sid`,`psdesc`,`pgms`,`pprice`,`date`,`status`,`stock`,`discount`,`popular`,`created_by`)values('".$pname."','".$url."','".$related."','".$sname."',".$catname.",".$subcatname.",'".$psdesc."','".$pgms."','".$pprice."','".$timestamp."',".$status.",".$ostock.",".$discount.",".$popular.",".$admin_id.")");
